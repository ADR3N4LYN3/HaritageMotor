...
QR tokens are UUIDs auto-generated by the backend when bays and vehicles are created. They are used for QR code scanning to quickly resolve a physical asset to its database record via `GET /scan/{token}`.

- Tokens are generated server-side (UUID v4), never user-supplied
- Each bay and vehicle gets a unique `qr_token` at creation time
- The scan endpoint resolves the token to either a vehicle or bay record

### Email Template XSS Sanitization

All user-provided inputs (name, email, company, vehicles, message) are escaped with `html.EscapeString()` before being injected into email HTML templates (`internal/service/contact/templates.go`). This prevents stored XSS via malicious form submissions that could execute in email clients.
